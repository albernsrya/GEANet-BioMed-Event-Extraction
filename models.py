...
from torch.nn import BCELoss, CrossEntropyLoss, MSELoss, Parameter
...
try:
    from torch_geometric.nn import (GATConv, GatedGraphConv, GCNConv,
                                    GraphConv, GraphUNet, NNConv, SAGEConv,
                                    TopKPooling)
except:
    pass
...
class EdgeNet(nn.Module):
    ...
    def __init__(
        ...
        edge_embedding_dim,
        output_dim,
        kg_pretrained_weights=None,
    ):
        super(EdgeNet, self).__init__()
        # self.linear1 = nn.Linear(num_edge_features, num_edge_features)

        if kg_pretrained_weights is not None:
            edge_embedding_weights = torch.load(
                kg_pretrained_weights)["rel_embeddings.weight"]
            self.edge_embedding = nn.Embedding.from_pretrained(
                edge_embedding_weights, freeze=False)
        else:
            self.edge_embedding = nn.Embedding(
                num_embeddings=num_edge_embeddings,
...
class KnowledgeGNN(nn.Module):
    def __init__(
        ...
        class ECGAT(NNConv):
            ...
            def message(self, edge_index_i, x_i, x_j, pseudo):
                ...
                weight = self.nn(pseudo).view(-1, self.in_channels,
                                              self.out_channels)
                neighbors = torch.matmul(x_j.unsqueeze(1),
        ...
        self.edge_mlp = EdgeNet(num_edge_embeddings, kg_embedding_dim,
                                kg_embedding_dim * kg_embedding_dim)

        if args.gnn_type == "ECGAT":
            self.conv1 = ECGAT(kg_embedding_dim, kg_embedding_dim,
                               self.edge_mlp)
            self.conv2 = ECGAT(kg_embedding_dim, kg_embedding_dim,
                               self.edge_mlp)
        if args.gnn_type == "NNConv":
            self.conv1 = NNConv(kg_embedding_dim, kg_embedding_dim,
                                self.edge_mlp)
            self.conv2 = NNConv(kg_embedding_dim, kg_embedding_dim,
                                self.edge_mlp)
        ...
        if kg_pretrained_weights is not None:
            kg_embedding_weights = torch.load(
                kg_pretrained_weights)["ent_embeddings.weight"]
            self.kg_embedding = nn.Embedding.from_pretrained(
                kg_embedding_weights, freeze=False)
        else:
            self.kg_embedding = nn.Embedding(
                num_embeddings=args.num_node_embeddings,
    ...
    def forward(
        ...
        # token, CUI, STY
        all_node_embeddings = torch.cat([token_embeddings, node_embeddings],
                                        dim=0)
        ...
        if self.training and args.mnc:
            ...
            if (~node_mask).sum() > 0:
                ...
                token_node_mask = torch.ones([token_embeddings.size(0)
                                              ]).bool().cuda()
                ...
                node_mask = torch.cat([node_mask.unsqueeze(1)] *
        ...
        # KGE loss
        edge_embeddings = torch.matmul(edge_attr,
                                       self.edge_mlp.edge_embedding.weight)
...
class MultitaskClassifierBase(nn.Module):
    ...
    def forward(
        ...
        if args.use_knowledge:
            for batch, kb_feature in enumerate(kg_datas):
                ...
                if len(kb_feature["nodes"]) > 0:
                    # place tensor to GPU
                    kb_feature = {
                        k: v.cuda() if k not in [
                            "CUI_length", "num_recognized_tokens", "tokens"
                        ] else v
                        for k, v in kb_feature.items()
                    }
                    nodes, edge_index, edge_attr = (
                        kb_feature["nodes"],
                        kb_feature["edge_index"],
                        kb_feature["edge_attr"],
                    )
                    ...
                    if args.use_temporal_edge:
                        ...
                        empty_attr = torch.zeros([edge_attr.size(0), 1],
                                                 dtype=torch.float).cuda()
                        edge_attr = torch.cat([edge_attr, empty_attr], dim=1)
                        sent_token_idx2graph_token_idx = {
                            sent_token_idx.item(): graph_token_idx
                            for graph_token_idx, sent_token_idx in enumerate(
                                token_ids)
                        }
                        token_relation_edge_index = [
                            [
                                sent_token_idx2graph_token_idx[l],
                                sent_token_idx2graph_token_idx[r],
                            ] for l, r in predicted_edges[batch]
                            if l in sent_token_idx2graph_token_idx
                            and r in sent_token_idx2graph_token_idx
                        ]

                        if len(token_relation_edge_index) > 0:
                            # [2, edges]
                            token_relation_edge_index = torch.tensor(
                                token_relation_edge_index).T.cuda()
                            edge_index = torch.cat(
                                [edge_index, token_relation_edge_index], dim=1)

                            # num_edge, edge_embedding_size
                            temporal_edge_attr = torch.zeros(
                                [
                                    token_relation_edge_index.size(1),
                                    args.num_edge_embeddings,
                                ],
                                dtype=torch.float,
                            ).cuda()
                            ...
                            edge_attr = torch.cat(
                                [edge_attr, temporal_edge_attr], dim=0)
                    ...
                    node_type_labels = (
                        [0] * num_recognized_tokens + [1] * CUI_length + [2] *
                        (len(nodes) - num_recognized_tokens - CUI_length))
                    node_type_labels = torch.tensor(node_type_labels,
                                                    dtype=torch.long).cuda()

                    token_embeddings = out[batch, token_ids, :]

                    node_embeddings, loss, node_type_loss, mnc_loss = self.gnn(
                        node_ids,
                        edge_index,
                        edge_attr,
                        token_embeddings,
        ...
        if task == "entity":
            ...
            protein_mask = torch.cat(out.size(2) *
                                     [(entity_labels == 9).unsqueeze(2)],
                                     dim=2)
            protein_embeddings = out * protein_mask

            protein_embeddings = F.avg_pool1d(
                protein_embeddings.permute(0, 2, 1),
                kernel_size=protein_embeddings.size(1),
                count_include_pad=False,
            ).permute(0, 2, 1)

            protein_embeddings = torch.cat(out.size(1) * [protein_embeddings],
                                           dim=1)
        ...
        if task == "relation":
            ...
            # out : [2, 114, 200]
            ltar_f = torch.cat(
                [out[b, lidx[b][r], :].unsqueeze(0) for b, r in rel_idxs],
                dim=0)

            rtar_f = torch.cat(
                [out[b, ridx[b][r], :].unsqueeze(0) for b, r in rel_idxs],
                dim=0)
    ...
    def construct_relations(
        ...
        # Case 2: use candidate relation predicted by entity model
            ...
            for i in range(len(predicted_entities)):
                ...
                tri_idxs = np.where((predicted_entity > 0) *
                                    (predicted_entity != 9))[0].tolist()
                ...
                tt_pairs = [
                    (i, j) for i in tri_idxs for j in tri_idxs if i != j
                    and args._id_to_label_t[predicted_entity[i]] in args.REG
                ]
                ...
                if not test:
                    ...
                    for p in te_pairs + tt_pairs:
                        ...
                        if _is_gold(p, interaction):
                            ...
                            pred_int.append(
                                interaction_label[interaction.index(p)])
                        else:
                            # None event
        ...
        if rels == []:
            ...
            labels = torch.LongTensor(
                [x for rel in interaction_labels for x in rel])
    ...
    def predict(
        ...
        gold,
        args,
        dev_interactions=None,
        dev_interaction_labels=None,
        test=False,
        eval_kg_datas=None,
    ):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(
                    tqdm(dev_dataloader, desc="Prediction")):
                ...
                # entity output
                entity_logits, prob_e, _, _, _ = self.forward(
                    dev_input_ids,
                    dev_entity_labels,
                    dev_input_masks,
                    dev_segment_ids,
                    ...
                    task="entity",
                    args=args,
                )  # out_e and prob_e: [16, 56, 11]

                # mask out the prob of the padding with input mask
                mask = torch.cat([dev_input_masks.unsqueeze(2)] *
                                 (entity_logits.size(2)),
                                 dim=2)
                mask[:, :, 0] = 1
                prob_e *= mask
                ...
                if not test:
                    gold_interactions = [
                        dev_interactions[sample_id]
                        for sample_id in dev_sample_ids
                    ]
                    gold_interaction_labels = [
                        dev_interaction_labels[sample_id]
                        for sample_id in dev_sample_ids
                    ]

                    all_gold_interactions.extend(gold_interactions)
                    all_gold_interaction_labels.extend([
                        args._id_to_label_i[label]
                        for labels in gold_interaction_labels
                        for label in labels
                    ])
                    # construct relation
                    label_r, rel_idxs, lidx, ridx = self.construct_relations(
                        prob_e,
                        dev_entity_labels,
                        dev_input_masks,
                        gold_interactions,
                ...
                pair_lengths = [len(i) for i in lidx
                                ]  # num of pairs in each sent in the batch
                for i in range(len(lidx)):  # batch size
                    if len(lidx[i]) == 0:
                        predicted_interactions.append([])
                    else:
                        predicted_interactions.append(
                            [i for i in zip(lidx[i], ridx[i])])

                # predict relations
                if rel_idxs != []:  # predicted relation could be empty --> skip

                    out_r, prob_r, _, _, _ = self.forward(
                        dev_input_ids,
                        dev_input_masks,
                        dev_segment_ids,
                        kg_datas=kg_datas,
                    ...
                    for i, l in enumerate(pair_lengths):
                        if pair_lengths[i] == 0:
                            ...
                            predicted_interaction_labels.append([
                                args._id_to_label_i[x]
                                for x in pred_r_list[cur:cur + l]
                            ])
                            cur += l
                    ...
                    predicted_interaction_labels.extend(
                        [[] for _ in range(len(dev_input_masks))])

                assert len(predicted_interaction_labels[-1]) == len(
                    predicted_interactions[-1])
                ...
                for i, mask in enumerate(dev_input_masks):
                    ...
                    ent_pred.append(
                        torch.masked_select(prob_e[i].argmax(dim=1), mask))

                    # flatten entity label
                    ent_label.append(
                        torch.masked_select(dev_entity_labels[i], mask))

                    ent_input.append(
                        torch.masked_select(dev_input_ids[i], mask))
...
class BertMultitaskClassifier(MultitaskClassifierBase):
    def __init__(
        self,
        args,
        bert_weights_path="biobert_weights/scibert_scivocab_uncased",
        kg_pretrained_weights=None,
    ):
        ...
        self.linear1_ent = nn.Linear(config.hidden_size * 2,
                                     args.ent_linear_size)
        self.linear2_ent = nn.Linear(args.ent_linear_size,
