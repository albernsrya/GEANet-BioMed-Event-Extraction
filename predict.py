...
def main(args, tmp_file_dir):
    ...
    if args.do_test:
        ...
        write_pkl(args,
                  ...
                  merged_pkl,
                  gold_tri=False)
        unmerge_normalize(args, tmp_file_dir,
                          args.output_dir.split("/")[-1], merged_pkl)
...
def unmerge_normalize(args, tmp_file_dir, model_name, merged_pkl):
    ...
    unmerge_cmd = (
        f"python unmerg_write.py -pred_pkl={merged_pkl}"
    ...
    # normalization
    normalize_cmd = (
        f"./eval/tools/a2-normalize.pl -g {tmp_file_dir}/ -u {tmp_file_dir}/*.a2"
    )
...
def create_json_output(tmp_file_dir, doc_id):
    ...
    def same_event(event1, event2):
        return (event1["triggers"] == event2["triggers"]
                and event1["event_type"] == event2["event_type"]
                and event1["arguments"] == event2["arguments"])
    ...
    for line in lines:
        ...
        if line[0].startswith("T"):
            ...
            entity_map[line[0]] = [
                int(line[2]), int(line[3]), " ".join(line[4:])
            ]
    ...
    for line in lines:
        if line[0].startswith("E"):
            ...
            current_event["triggers"] = [{
                "event_type":
                event_type,
                "text":
                entity_map[entity_id][2],
                "start_token":
                char2doctoken_map[list(char2doctoken_map.keys())[bisect_right(
                    list(char2doctoken_map.keys()), entity_map[entity_id][0]) -
                    1]],
                ...
                "end_token":
                char2doctoken_map[list(char2doctoken_map.keys())[bisect_right(
                    list(char2doctoken_map.keys()),
                    entity_map[entity_id][1] - 1,
                ) - 1]],
            }]
            ...
            for entity in line[2:]:
                ...
                current_argumet = {
                    ...
                    entity_map[entity_id][2],
                    "start_token":
                    char2doctoken_map[list(char2doctoken_map.keys())[
                        bisect_right(list(char2doctoken_map.keys()),
                                     entity_map[entity_id][0]) - 1]],
                    # the first -1 is for go back one character because GENIA annotation at the character level is exlusive at the end i.e. [start, end)
                    "end_token":
                    char2doctoken_map[list(
                        char2doctoken_map.keys())[bisect_right(
                            list(char2doctoken_map.keys()),
                            entity_map[entity_id][1] - 1,
                        ) - 1]],
                }
...
def biomedical_evet_extraction(user_input):
    ...
    doc_id = hashlib.sha256(str(
        time.time()).encode("utf-8")).hexdigest() + ".d0"
    args.doc_id = doc_id
