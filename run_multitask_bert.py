...
def main(args):
    ...
    data_test = pickle.load(
        open(os.path.join(args.data_dir, args.test_pkl), "rb"))

    test_kg_datas = None
    if args.use_knowledge:
        test_kg_datas = pickle.load(
            open(f"{args.data_dir}/{args.test_kg_datas}.pkl", "rb"))
    ...
    args._label_to_id_t = OrderedDict([
        ("None", 0),
        ("Gene_expression", 1),
        ("Localization", 2),
        ("Transcription", 3),
    ...
    args._id_to_label_t = {
        ...
        4: "Binding",
        5: "Phosphorylation",
        6: "Positive_regulation",
        7: "Regulation",
        8: "Protein_catabolism",
        9: "Protein",
        10: "Negative_regulation",
    }
    args._label_to_id_i = OrderedDict([("None", 0), ("Theme", 1),
                                       ("Cause", 2)])
    args._id_to_label_i = {0: "None", 1: "Theme", 2: "Cause"}
    args.SIMPLE = [
        "Gene_expression",
    ...
    test_entity_labels = torch.tensor(
        [[args._label_to_id_t[entity_label] for entity_label in entity_labels]
         for entity_labels in data_test["entity_labels"]],
        dtype=torch.long,
    )
    ...
    if args.do_test:
        model = BertMultitaskClassifier(args,
        ...
        model.eval()
        gold = False
        y_trues_e, y_preds_e, y_trues_r, y_preds_r, data_out = model.predict(
            test_dataloader,
            gold,
            args,
            test=True,
            eval_kg_datas=test_kg_datas)
...
def unmerge_normalize(model_name):
    ...
    unmerge_cmd = (
        f"python unmerg_write.py -pred_pkl={args.out_test_pkl}"
    ...
    # normalization
    normalize_cmd = (
        f"./eval/tools/a2-normalize.pl -g genia_cord_19/ -u {output_dir}/*.a2")
    p = subprocess.Popen(normalize_cmd, stdout=subprocess.PIPE, shell=True)
    output, err = p.communicate()

    # copy a1 & txt files to a2
    [
        shutil.copy(source_file, output_dir) for file in os.listdir(output_dir)
        for source_file in [
            f'genia_cord_19/{file.split(".a2")[0]}.txt',
            f'genia_cord_19/{file.split(".a2")[0]}.a1',
        ]
    ]
...
def write_pkl(data_out, abs_spans, doc_ids, out_file, gold_tri=True):
    ...
    gold_entities = [
        np.array([args._id_to_label_t[entity] for entity in entities],
                 dtype=object) for entities in gold_entities
    ]
    predicted_etities = [
        np.array([args._id_to_label_t[entity] for entity in entities],
                 dtype=object) for entities in predicted_etities
    ]

    assert (len(predicted_interactions) == len(predicted_interaction_labels) ==
            len(predicted_interactions) == len(gold_entities) == len(abs_spans)
            == len(doc_ids)), (
                len(predicted_interactions),
                len(predicted_interaction_labels),
                len(predicted_interactions),
                len(gold_entities),
                len(abs_spans),
                len(doc_ids),
    ...
    for i in range(len(gold_entities)):
        ...
        if gold_tri:
            out = tuple([
                ...
                None,
                gold_entities[i],
                predicted_interactions[i],
                predicted_interaction_labels[i],
                abs_spans[i],
                None,
                None,
            ])
        ...
        assert len(abs_spans[i]) == len(gold_entities[i]), (
            len(abs_spans[i]),
            len(gold_entities[i]),
        )
...
if __name__ == "__main__":
    ...
    p.add_argument("-model", type=str, default="multitask/pipeline"
                   )  # , 'multitask/gold', 'multitask/pipeline'
    ...
    p.add_argument(
        "-pipe_epoch", type=int,
        default=1000)  # 1000: no pipeline training; otherwise <= epochs
    # p.add_argument('-seed', type=int, default=123)
    p.add_argument("-lr", type=float, default=5e-5)
    p.add_argument("-num_classes", type=int,
                   default=2)  # get updated in main()
    ...
    p.add_argument(
        "-entity_model_file",
        type=str,
        default="singletask_trigger_EXPtest_TF10.7015_RF10.3919_WF10.7015.pth.tar",
    )
    p.add_argument(
        "-relation_model_file",
        type=str,
        default="singletask_relation_EXPtest_TF10.0048_RF10.8791_WF10.8791.pth.tar",
    )
    p.add_argument(
        "-entity_model_file_e2e",
        type=str,
        default="singletask_end2end_trigger_EXPtest_TF10.7020_RF10.8633_WF10.8296.pth.tar",
    )
    p.add_argument(
        "-relation_model_file_e2e",
        type=str,
        default="singletask_end2end_relation_EXPtest_TF10.7020_RF10.8633_WF10.8296.pth.tar",
    )
    ...
    p.add_argument("-nw_r",
                   type=float,
                   default=1.0,
                   help="weight for None in relation")
    p.add_argument("-nw_e",
                   type=float,
                   default=1.0,
                   help="weight for None in trigger")
    p.add_argument("-regen_vocfile", type=str2bool, default=False)
    p.add_argument(
        "-do_eval",
        type=str2bool,
        default=False,
        help="load trained models and predict to write pkls, \
    ...
    p.add_argument("-do_train",
                   type=str2bool,
                   default=False,
                   help="true to train the model")
    p.add_argument("-do_test",
                   type=str2bool,
                   default=False,
                   help="true to predict on the test set")
    p.add_argument("-test_on_gold_tri", type=str2bool, default=True)
    p.add_argument(
        "-tune_t",
        type=str2bool,
        default=True,
        help="used with load_model, decide wether the loaded trigger model will be updated during end2end",
    )
    ...
    p.add_argument(
        ...
        type=str2bool,
        required=True,
        help="Enable node type classification loss",
    )
    p.add_argument(
        "-edge_cls",
        type=str2bool,
        default=False,
        help="Enable edge classification loss",
    )
    p.add_argument(
        "-mnc",
        type=str2bool,
        required=True,
        help="Enable masked node classification loss",
    )
    ...
    args = p.parse_args()
    args.save_stamp = "%s_hid%s_dropout%s_ew%s" % (
        args.save_stamp,
        args.hid,
        args.dropout,
        args.entity_weight,
