...
def generate_sentence_instance(document, sentence, doc_id, ent_char2_id_map,
                               corpus_proteinOrigIdBySpan):
    """
    ...
    if len(entities) > 0:
        segment_numbers = ([0] + np.hstack(
            [[entity.start_char - sent_start, entity.end_char - sent_start]
             for entity in entities]).tolist() + [len(text) + 1])
    else:
        segment_numbers = [0]

    sub_sentences = [
        text[start:end]
        for start, end in zip(segment_numbers[:-1], segment_numbers[1:])
    ]
    ...
    for sub_sentence in sub_sentences:
        ...
        for original_texts in re.split(skip_patterns, sub_sentence):
            ...
            while (text[tail_char_position] in [" "]
                   and tail_char_position >= head_char_position):
                tail_char_position -= 1

            # this is invalid
            if (tail_char_position < head_char_position
                    or re.search(skip_patterns, original_texts)
                    is not None):  # original_texts=='\n':#
            ...
            if new_tokens[tail_position - 1] not in ["[UNK]"]:
                while (text[tail_char_position].lower() !=
                       new_tokens[tail_position - 1][-1]):
                    # tail token position
                    tail_position -= 1

            char_to_token_tail_map[tail_char_position] = len(new_tokens)
            abs_span = "-".join([
                str(head_char_position + sent_start),
                str(tail_char_position + 1 + sent_start),
            ])
    ...
    entity_label = np.array(["None"] * (len(new_tokens) + 1),
                            dtype=object)  # plus 1 for [SEP]
    ...
    for entity in entities:
        ...
        corpus_proteinOrigIdBySpan[doc_id][abs_spans[char_to_token_map[
            head_char_idx]]] = f"{doc_id}.{ent_char2_id_map[entity.start_char]}"

        # make sure at least the first character of the mapping is correct
        assert (new_tokens[char_to_token_map[head_char_idx]][0] ==
                text[head_char_idx].lower()), (
                    new_tokens[char_to_token_map[head_char_idx]][0],
                    text[head_char_idx],
                    entity,
        )
    ...
    assert (len(tokenized_ids) == max_length == len(mask_ids) ==
            len(segment_ids) == len(entity_label)), (
                len(tokenized_ids),
                max_length,
                len(mask_ids),
                len(segment_ids),
...
def process_document(document, doc_id, output_dir, corpus_proteinOrigIdBySpan):
    ...
    char2doctoken_map = {
        tok.idx: doc_tok_id
        for doc_tok_id, tok in enumerate(document)
    }

    # ner for each sentence.
    # -sent.start to convert to sentence-level anntation
    ner = [[ent.start, ent.end - 1, tokidx2ent_map[ent.start]]
           for ent in document.ents]

    # create mapping from starting character position to entity id
    ent_char2_id_map = {
        ent.start_char: f"T{idx+1}"
        for idx, ent in enumerate(document_entities)
    }

    with open(f"{output_dir}/{doc_id}.a1", "w") as f:
        [
            f.write(
                f"{ent_char2_id_map[ent.start_char]}\tProtein {ent.start_char} {ent.end_char}\t{ent}\n"
            ) for ent in document_entities
        ]
    doc_result_dict = []
    for sentence in document.sents:
        doc_result_dict.append(
            generate_sentence_instance(document, sentence, doc_id,
                                       ent_char2_id_map,
                                       corpus_proteinOrigIdBySpan))
...
def preprocess_input(document, doc_id, output_dir):
    ...
    doc_result_dict, preprocess_result = process_document(
        document, doc_id, output_dir, corpus_proteinOrigIdBySpan)
    ...
    all_result_dict["sample_ids"] = np.arange(
        len(all_result_dict["tokenized_ids"])).tolist()
