...
def writeA2(orig_docid, args, triggerIdBySpan, triggerTypeBySpan, all_events):
    triggerIdBySpan = OrderedDict(
        sorted(list(triggerIdBySpan.items()),
    ...
    if len(all_events) == 0 and len(triggerIdBySpan) == 0:
        return
    ...
    with open("{}/{}.a2".format(args.out_dir, orig_docid), "w") as f:
        ...
        for span, trigger_id in list(triggerIdBySpan.items()):
            ...
            T_line = "{}\t{} {} {}\t{}\n".format(
                trigger_id,
                triggerTypeBySpan[span],
                span_l,
                span_r,
                origial_texts[int(span_l):int(span_r)],
            )
            f.write(T_line)
        ...
        for event in sorted(
                all_events,
                key=lambda event: triggerIdBySpan[event["trigger_span"]]):

            arg_str = " ".join([
                ":".join([k, v]) for (k, v) in sorted(
                    list(event.items()), reverse=True, key=lambda x: x[0])
                if k.startswith("Theme") or k == "Cause"
            ])
            E_line = ("{}\t{}:{} ".format(
                event["ST_id"],
                event["trigger_type"],
                triggerIdBySpan[event["trigger_span"]],
            ) + arg_str + "\n")
            f.write(E_line)
...
def check_newevent(event):
    if "Cause" in event:
        if (not event["Cause"].startswith("T")) and (
                not event["Cause"].startswith("E")):
            return False
    for k, v in list(event.items()):
        if k.startswith("Theme"):
            if (not event[k].startswith("T")) and (
                    not event[k].startswith("E")):
                return False
    return True
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str2bool,
        default=True,
        help="apply the heuristics of taking the longest chain for unmerging",
    )
    p.add_argument("-pred_pkl", type=str, default="CORD_out_merged.pkl")
    p.add_argument(
        "-protIdBySpan",
        type=str,
        default="preprocessed_data/CORD_19_PMCprotIdBySpan.pkl",
    )
    p.add_argument("-origIdById",
                   type=str,
                   default="preprocessed_data/CORD_19_PMCorigIdById.pkl")
    ...
    for doc_id in list(protIdBySpan.keys()):  # doc_id: GE09.d1
        ...
        trigger_id = 1 + max(
            [int(i.split(".")[-1][1:]) for i in list(doc_prots.values())])
                ...
                for cur_comb in cur_combs:
                    ...
                    for edge in cur_comb:
                        ...
                        if target_span in triggerIdBySpan and target_span in doc_prots:
                            ...
                            event[arg_role] = doc_prots[target_span].split(
                                ".")[-1]
                    ...
                    if is_terminal(event):
                        event["ST_id"] = "E{}".format(
                            event_id)  # Assign event ids for terminal events
                        eventIdsBySpan[spans[idx]].append(event["ST_id"])
                        event_id += 1
                    else:
                        event[
                            "ST_id"] = "X"  # Nesting events, Id To be determined later
                    # Think about this!!!!!!!!!!!
        ...
        while event_cand_stack:
            ...
            for idx in range(len(event_cand_stack)):
                ...
                if cause_target_span:
                    if theme_target_ids is not None and cause_target_ids is not None:
                        ...
                        for i in range(len(new_combs)):
                            new_event = {}
                            new_event["trigger_type"] = cur_event[
                                "trigger_type"]
                            new_event["trigger_span"] = cur_event[
                                "trigger_span"]
                            ...
                            eventIdsBySpan[cur_event["trigger_span"]].append(
                                new_event["ST_id"])
                            event_id += 1
                ...
                #     pdb.set_trace()
            event_cand_stack = [
                event_cand_stack[i] for i in range(len(event_cand_stack))
                if remove[i] == False
            ]
        ...
        all_events = [
            event for event in events + new_events if event["ST_id"] != "X"
        ]
        ...
        for event in all_events:
            if event["trigger_type"] not in REG:
                ...
                for k, v in list(event.items()):
                    if k.startswith("Theme"):
                        if not (event[k].startswith("T")
                                or event[k].startswith("E")):
                            raise AssertionError
            if (not event["Theme"].startswith("T")) and (
                    not event["Theme"].startswith("E")):
                if event["trigger_type"] not in REG:
                    raise AssertionError(pdb.set_trace())
            for k, v in list(event.items()):
                if k.startswith("Theme") or k == "Cause":
        ...
        writeA2(orig_docid, args, triggerIdBySpan, triggerTypeBySpan,
                all_events)
